...
- Risky categories shipped with item-level review: old downloads (`old-downloads`), large files (`large-files`), and content-hashed duplicates (`duplicate-files`). All three are High risk: TUI starts them unchecked, large-files/duplicate-files are picker-only (no bulk apply), and duplicates keep one copy per group by default.
