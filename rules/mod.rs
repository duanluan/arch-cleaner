...
use crate::model::{
    CleanerOptions, CleanupCommand, CleanupTarget, LocalizedText, RiskLevel, ScanItem, ScanReport,
...
/// 把用户在扫描结果页勾选的条目变成一个可执行的清理目标。
/// 安全约束：只接受绝对路径（扫描产物本来就是绝对路径，这里再挡一道），
/// 每条命令都是 `rm -rf -- <path>` 的 argv 形式——不经 shell、不吃通配符；
/// 重复路径去重。
pub fn item_removal_target(items: &[ScanItem], needs_sudo: bool) -> Result<CleanupTarget, String> {
    if items.is_empty() {
    ...
    for item in items {
        if !item.path.is_absolute() {
            return Err(format!(
                "refusing to remove relative path: {}",
                item.path.display()
            ));
        }
        ...
        let path_text = item.path.to_string_lossy().to_string();
        ...
        args.extend(["-rf", "--"].into_iter().map(String::from));
        args.push(path_text.clone());

        commands.push(CleanupCommand::new(
            format!("{program} rm -rf -- {}", path_display(&item.path)),
            program,
            args,
            needs_sudo,
        ));
    ...
    Ok(CleanupTarget {
        ...
        description: LocalizedText {
            zh_cn: "仅删除在扫描结果中勾选的条目，其余内容不动。",
            en: "Remove only the entries selected in the scan results; everything else stays.",
        },
        ...
        threshold_summary: |_options: &CleanerOptions, language: Language| {
            i18n::tr(
                language,
                "逐项删除，无统一阈值",
                "Per-item removal; no shared threshold",
            )
            .to_string()
        },
        scan: scan_selected_items,
        dry_run_commands: Vec::new(),
        apply_commands: commands,
    })
}

fn scan_selected_items(
    target: &CleanupTarget,
    ...
    report.warnings.push(
        i18n::tr(
            language,
            "此目标由文件选择器动态生成，条目以扫描结果为准。",
            "This target is generated by the file picker; its entries come from scan results.",
        )
        .to_string(),
    );
    report
...
fn cleanable_files_size(path: &Path, min_age_days: u16) -> io::Result<ScanItem> {
...
mod tests {
    ...
    #[test]
    fn item_removal_builds_safe_deduped_rm_commands() {
        ...
        let items = vec![
            ...
            ScanItem {
                path: PathBuf::from("/tmp/b c"),
                bytes: 5,
                entries: 2,
            },
        ];
        let target = super::item_removal_target(&items, false).unwrap();
        assert_eq!(target.apply_commands.len(), 2);
        for command in &target.apply_commands {
            assert_eq!(command.program, "rm");
            // argv 直传：路径带空格也只是一整个参数，不进 shell。
            assert_eq!(command.args[0], "-rf");
            assert_eq!(command.args[1], "--");
        }
        assert!(target.apply_commands[1].args[2].contains("b c"));

        let sudo_target = super::item_removal_target(&items[..1], true).unwrap();
        let command = &sudo_target.apply_commands[0];
        assert_eq!(command.program, "sudo");
        assert_eq!(command.args[0], "rm");

        assert!(super::item_removal_target(&[], false).is_err());
        let relative = vec![ScanItem {
            path: PathBuf::from("tmp/relative"),
            bytes: 1,
            entries: 1,
        }];
        assert!(super::item_removal_target(&relative, false).is_err());
    }
